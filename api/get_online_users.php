...
function get_online_users()
{
  ...
  if ($page > 1 || $total > 0)
  {
    $query_2 = "SELECT u_openid, u_nickname, u_sex, u_city, u_province, u_country, u_headimgurl, u_subscribe_time, u_unionid, l_latitude, l_longitude FROM login_l LEFT JOIN users_u ON l_openid=u_openid LIMIT ".strval($start).",".strval($per_page);
    ...
    while ($row = mysqli_fetch_array($result))
    {
      ...
      $users = $users.",{\"oi\":".jsonstr($u_openid).",\"nn\":".jsonstr($u_nickname).",\"sx\":".jsonstrval($u_sex).",\"ct\":".jsonstr($u_city).",\"pr\":".jsonstr($u_province).",\"cn\":".jsonstr($u_country).",\"hd\":".jsonstr($u_headimgurl).",\"st\":".jsonstr($u_subscribe_time).",\"ui\":".jsonstr($u_unionid).",\"lat\":".jsonstrval($l_latitude).",\"lng\":".jsonstrval($l_longitude)."}";
    }
    mysqli_free_result($result);
